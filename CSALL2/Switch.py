...
class Dashboard(tk.Frame):
    def __init__(self, master, controller):
        ...
        self.try_b = tk.Button(self.side_frame, text='try', relief='flat', command=self.draw)
        self.try_b.place(x=700, y=60)
        """self.chart_frame = Frame(self.side_frame, bg='white', width=1280, height=720)
        self.chart_frame.place(x=0, y=100)"""

    def draw(self):
        category_in_total_amount = pd.read_sql_query(
            "SELECT ty.type_name AS Type, c.cat_name AS Category, sum(t.amount) AS Amount FROM transactions t, category c, type ty WHERE c.cat_id = t.cat_id AND t.user_id = c.user_id AND t.type_id = ty.type_id AND t.type_id = 1 AND t.user_id = 1 GROUP BY t.cat_id",
            connect)
        ...
        category_ex_total_amount = pd.read_sql_query(
            "SELECT ty.type_name AS Type, c.cat_name AS Category, sum(t.amount) AS Amount FROM transactions t, category c, type ty WHERE c.cat_id = t.cat_id AND t.user_id = c.user_id AND t.type_id = ty.type_id AND t.type_id = 2 AND t.user_id = 1 GROUP BY t.cat_id",
            connect)
        ...
        can.draw()
        can.get_tk_widget().place(x=10, y=10)"""
        """toolb = NavigationToolbar2Tk(can, self.chart_frame)
        toolb.update()
