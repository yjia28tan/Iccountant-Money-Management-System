...
class UserAccount(tk.Frame):
    ...
    def editUsername(self):
        ...
        self.EditUsernameEntry = Entry(self.editUsernameWindow, width=30, fg='white', bg='#1A1A1A', justify=CENTER,relief=FLAT, insertbackground = 'white')
        ...
        self.PasswordEntry = Entry(self.editUsernameWindow, show = '*', width=30, fg='white', bg='#1A1A1A', justify=CENTER,relief=FLAT, insertbackground = 'white')
        ...
        #show password
        self.btn_value = IntVar(value=0)
        self.check_btn = Checkbutton(self.editUsernameWindow, text="Show password", variable=self.btn_value, command=self.show_password_username, fg = 'white', bg='#1A1A1A')
        self.check_btn.pack(pady = 5)
    ...
    def editusername(self):
        if not self.EditUsernameEntry.get() or not self.PasswordEntry.get():
            ...
            cursor.execute('SELECT password from user where password=? and user_id = ?',(self.PasswordEntry.get(),self.controller.shared_user_id['userID'].get(),))
            #fetch data 
            if cursor.fetchone():
                # edit values in database
    ...
    def editPassword(self):
        ...
        self.up = Frame(self.editPasswordWindow, bg = '#1A1A1A')
        
        self.up.pack()

        # page title
        self.EditPasswordHeading = Label(self.up, text='Edit Password', fg='white', bg='#1A1A1A',
                                           font=tkFont.Font(family='calibri', size=20))
        self.EditPasswordHeading.pack(pady=10)

        #current password
        self.CurrentPasswordLabel = Label(self.up, text='Current Password', fg='white', bg='#1A1A1A',
                                               font=tkFont.Font(family='calibri', size=15))
        self.CurrentPasswordLabel.pack(pady=10)
        self.CurrentPasswordEntry = Entry(self.up,  show='*',width=30, fg='white', bg='#1A1A1A',  insertbackground = 'white',justify=CENTER,relief=FLAT)
        self.CurrentPasswordEntry.pack(pady=5)
        self.CurrentPassword_line = Canvas(self.up, width=200, height=2.0, bg="#bdb9b1", highlightthickness=0)
        self.CurrentPassword_line.pack()

        #new password
        self.EditPasswordLabel = Label(self.up, text='New Password', fg='white', bg='#1A1A1A',
                                               font=tkFont.Font(family='calibri', size=15))
        self.EditPasswordLabel.pack(pady=10)
        
        self.EditPasswordEntry = Entry(self.up,  show='*',width=30, fg='white', bg='#1A1A1A', insertbackground = 'white', justify=CENTER,relief=FLAT)
        self.EditPasswordEntry.pack(pady=5)
        self.EditPassword_line = Canvas(self.up, width=200, height=2.0, bg="#bdb9b1", highlightthickness=0)
        self.EditPassword_line.pack()

        #confirm password
        self.ConfirmPasswordLabel = Label(self.up, text='Confirm New Password', fg='white', bg='#1A1A1A',
                                               font=tkFont.Font(family='calibri', size=15))
        self.ConfirmPasswordLabel.pack(pady=10)
        
        self.ConfirmPasswordEntry = Entry(self.up,  show='*',width=30, fg='white', bg='#1A1A1A', insertbackground = 'white', justify=CENTER,relief=FLAT)
        self.ConfirmPasswordEntry.pack(pady=5)
        self.ConfirmPassword_line = Canvas(self.up, width=200, height=2.0, bg="#bdb9b1", highlightthickness=0)
        self.ConfirmPassword_line.pack()

        self.btn_value = IntVar(value=0)
        self.check_btn = Checkbutton(self.up, text="Show password", variable=self.btn_value, command=self.show_password, fg = 'white', bg='#1A1A1A')
        self.check_btn.pack(pady = 5)

        #send otp button
        self.sendOTP = customtkinter.CTkButton(self.up, text='send otp', width=50, height=30,
                                                  fg_color="#464E63",
                                                  hover_color="#667190", command= lambda:self.data_validation())
        self.sendOTP.pack(pady=10)

        self.down = Frame(self.editPasswordWindow, bg = '#1A1A1A')
        self.down.pack()
        
        #otp
        self.otpLabel = Label(self.down, text='4-digit OTP', fg='white', bg='#1A1A1A',
                                               font=tkFont.Font(family='calibri', size=15))
        self.otpLabel.pack(pady=10)
        self.otpEntry = Entry(self.down, show='*', width=30, fg='white', bg='#1A1A1A', insertbackground = 'white', justify=CENTER,relief=FLAT)
        self.otpEntry.pack(pady=5)
        self.otp_line = Canvas(self.down, width=200, height=2.0, bg="#bdb9b1", highlightthickness=0)
        self.otp_line.pack()
        ...
        #confirm button
        self.addConfirm = customtkinter.CTkButton(self.down, text='confirm', width=50, height=30,
                                                  fg_color="#464E63",
                                                  hover_color="#667190", command=lambda:self.otp_validation())
        self.addConfirm.pack(pady=10)
    ...
    def data_validation(self):
        ...
        if not self.currentp or not self.newp or not self.confirmnewp:
            ...
            cursor.execute('SELECT password from user where password=? and user_id = ?',(self.currentp,self.controller.shared_user_id['userID'].get(),))
            #fetch data 
            if cursor.fetchone():
    ...
    def otp_validation(self):
        ...
        if self.otpEntry.get() == self.OTP:
            ...
            #update data
            # edit values in database
            cursor.execute("UPDATE user SET password = ? WHERE user_id =?", (self.newp,self.controller.shared_user_id['userID'].get(),))
            # commit changes
            connect.commit()
            ...
            self.subject2 = "Icccountant account has successfully updated the password!"
            self.text2 = "Hi user,\n\nYour account for Icccountant desktop app has successfully reset the account password.\nThank you.\n\n\nIccountant"
            self.msg2 = 'Subject: {}\n\n{}'.format(self.subject2, self.text2)
